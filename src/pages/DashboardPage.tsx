...
export function DashboardPage({ transactions, txLoading, txLoadingMore = false, txHasMore = true, txError = null, publicKey, usdcBalance, xlmBalance, onRefresh, onLoadMore }: Props) {
        ...
        <div className="divide-y divide-white/4">
          {txLoading && transactions.length === 0 ? (
            ...
            <div className="text-center py-10 px-5">
              <p className="font-display text-xs text-red-400/70 tracking-widest">FAILED TO LOAD TRANSACTIONS</p>
              <p className="text-white/30 text-sm mt-2">{txError}</p>
              <button
                onClick={onRefresh}
                className="mt-4 px-4 py-2 rounded-lg border border-neon-cyan/20 text-xs font-display tracking-wider text-neon-cyan hover:bg-neon-cyan/5 transition-colors"
              >
                RETRY
              </button>
            </div>
            ...
            <>
              ...
              {/* Pagination footer */}
              <div className="flex flex-col items-center gap-3 px-5 py-4 border-t border-white/5">
                {txLoadingMore ? (
                  <div className="flex items-center gap-2 text-white/40">
                    <motion.div
                      ...
                      transition={{ duration: 0.8, repeat: Infinity, ease: 'linear' }}
                    />
                    <span className="font-display text-xs tracking-widest">LOADING OLDER TRANSACTIONS...</span>
                  </div>
                ) : txError ? (
                  <div className="flex flex-col items-center gap-2">
                    <p className="font-display text-xs text-red-400/60 tracking-widest">{txError}</p>
                    <button
                      onClick={onLoadMore ?? onRefresh}
                      className="px-4 py-1.5 rounded-lg border border-red-500/20 text-xs font-display tracking-wider text-red-400 hover:bg-red-500/5 transition-colors"
                    >
                      RETRY
                    </button>
                  </div>
                ) : !txHasMore ? (
                  <p className="font-display text-xs text-white/20 tracking-widest">END OF TRANSACTION HISTORY</p>
                ) : (
                  <button
                    onClick={onLoadMore}
                    disabled={!onLoadMore}
                    className="px-4 py-2 rounded-lg border border-white/10 text-xs font-display tracking-wider text-white/40 hover:text-neon-cyan hover:border-neon-cyan/30 transition-colors disabled:opacity-40"
                  >
